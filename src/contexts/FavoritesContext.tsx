...
export const FavoritesProvider: React.FC<FavoritesProviderProps> = ({ children }) => {
  ...
  // Убираем incrementFavorites и decrementFavorites - они вызывают ре-рендер
  ...
  const addToFavorites = (listing: Listing) => {
    ...
    setFavorites(prev => {
      if (!prev.find(fav => fav.id === listing.id)) {
        // НЕ вызываем incrementFavorites - это вызывает ре-рендер списков
        return [...prev, listing];
      }
      return prev;
    });
  ...
  const removeFromFavorites = (listingId: string) => {
    setFavorites(prev => {
      ...
      if (wasInFavorites) {
        // НЕ вызываем decrementFavorites - это вызывает ре-рендер списков
      }
